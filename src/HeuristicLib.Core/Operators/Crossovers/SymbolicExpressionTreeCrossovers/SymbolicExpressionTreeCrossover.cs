using HEAL.HeuristicLib.Genotypes.Trees;
using HEAL.HeuristicLib.SearchSpaces.Trees;

namespace HEAL.HeuristicLib.Operators.Crossovers.SymbolicExpressionTreeCrossovers;

/// <summary>
///   A base class for operators that perform a crossover of symbolic expression trees.
/// </summary>
public abstract record class SymbolicExpressionTreeCrossover : SingleSolutionStatelessCrossover<SymbolicExpressionTree, SymbolicExpressionTreeSearchSpace>;
